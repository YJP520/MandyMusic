...
PLAYINFO_X, PLAYINFO_Y = 16 * 14, 100  # 播放信息显示 x y 坐标
VOLUME_X, VOLUME_Y = 16 * 40, 5  # 音量文字 x y 坐标
TIME_X, TIME_Y = 16 * 10, 16 * 16  # 播放时间显示 x y 坐标
PLAYSTOP_X, PLAYSTOP_Y = 16 * 23, 16 * 20  # 播放暂停组件 x y 坐标
LAST_X, LAST_Y = 16 * 14, 16 * 20  # 上一曲组件 x y 坐标
NEXT_X, NEXT_Y = 16 * 32, 16 * 20  # 下一曲组件 x y 坐标
...
class MainMusic():
    ...
    def startGame(self):
        ...
        while True:
            ...
            MainMusic.window.blit(self.getTextSurface("%s" \
                % MainMusic.Musics_List[MainMusic.musicNumber][7:-4], \

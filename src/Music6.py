...
"""
    ...
    更新内容 :
        ...
        -
    需求：
        - 歌词显示需要定义专门的存储读取的文件，自定义格式{start, end, '这是一句歌词'}
        -
...
PLAY_MUSIC_FONT = 'dengxian'  # 歌曲名显示字体
PLAY_SINGER_FONT = 'dengxian'  # 歌手显示字体
PLAY_WORDS_FONT = 'dengxian'  # 歌词显示字体

SIZE_FONT = 24  # 主题字体大小
TIME_SIZE_FONT = 24  # 时间显示字体大小
PLAY_MUSIC_SIZE_FONT = 32  # 歌曲名显示字体大小
PLAY_SINGER_SIZE_FONT = 16  # 歌手显示字体大小
PLAY_WORDS_SIZE_FONT = 24  # 歌手显示字体大小

GETTEXT_X, GETTEXT_Y = 5, 5  # 提示文字 x y 坐标
PLAY_MUSIC_X, PLAY_MUSIC_Y = 16 * 20, 16 * 4 # 歌曲名显示 x y 坐标
PLAY_SINGER_X, PLAY_SINGER_Y = 16 * 20, 16 * 7 # 歌手显示 x y 坐标
PLAY_WORDS_X, PLAY_WORDS_Y = 16 * 20, 16 * 14 # 歌词显示 x y 坐标
...
class Music:
    def __init__(self, fileName):
        ...
        self.setVolume()  # 设置音量
        # 歌曲信息 默认格式：歌手_-_歌曲名.mp3
        start = 0  # 字符分隔符 编号
        indexLeft = fileName.rfind('\\', start) + 1  # 左边开始
        indexRight = fileName.find('-', start) - 1  # 右边结束
        # 获取歌手名
        self.singer = fileName[indexLeft: indexRight]
        indexLeft = indexRight + 3  # 更新左边
        indexRight = fileName.find('.', start)  # 右边结束
        # 获取歌曲名
        self.musicName = fileName[indexLeft: indexRight]
        print(self.singer)
...
class MainMusic():
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                ...
                if  event.button == 1:
                    ...
                    # 判断是否点击了设置按钮 - 判断是否鼠标的坐标在暂停播放按钮的坐标范围内
                        ...
                        # 设置主题
                        MainMusic.themeSetting += 1
                        # 归位循环切换
                        if MainMusic.themeSetting > 1:
                            MainMusic.themeSetting = 0
                    # 进度条某位置 普通点击
    ...
    def startMusic(self):
        ...
        # 让窗口持续刷新操作
        while True:
            # 给窗口一个填充色 默认MainMusic.themeSetting == 0
            ...
            # 播放信息 歌曲名 将绘制文字得到的小画布，放到窗口上
            MainMusic.window.blit(self.getTextSurface("%s" % MainMusic.music.musicName,
                 PLAY_MUSIC_FONT, PLAY_MUSIC_SIZE_FONT, COLOR_FONT), (PLAY_MUSIC_X, PLAY_MUSIC_Y))
            # 播放信息 歌手 将绘制文字得到的小画布，放到窗口上
            MainMusic.window.blit(self.getTextSurface("%s" % MainMusic.music.singer,
                 PLAY_SINGER_FONT, PLAY_SINGER_SIZE_FONT, COLOR_FONT), (PLAY_SINGER_X, PLAY_SINGER_Y))
            # 播放信息 一句歌词 将绘制文字得到的小画布，放到窗口上
            MainMusic.window.blit(self.getTextSurface("%s" % '开发者太懒(T＿T)，未加载歌词(*/ω＼*)',
                PLAY_WORDS_FONT, PLAY_WORDS_SIZE_FONT, COLOR_FONT),(PLAY_WORDS_X, PLAY_WORDS_Y))
